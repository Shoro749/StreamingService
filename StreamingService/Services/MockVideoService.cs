...
public class MockVideoService : IMoviesService, IFavoritesService
{
    private List<VideoCardViewModel> _videos = new List<VideoCardViewModel>
    {
        // 1. Дюна: Частина друга (Фільм)
            new VideoCardViewModel
            {
                ...
                Duration = "166 хв",
                AgeRating = "12+",
                TrailerUrl = "https://www.youtube.com/watch?v=DtR76pz517E",
        ...
        new VideoCardViewModel
        {
            Id = 2,
            Title = "Інтерстеллар",
            PosterUrl = "https://www.themoviedb.org/t/p/w1280/9d1sCoMSGJZtghS2X9us1h9u8lW.jpg",
            BackdropUrl = "https://media.themoviedb.org/t/p/w1000_and_h563_face/65BTgbR7w8g5h8PlNwUgRVWqPyQ.jpg",
            ThumbnailUrl = "https://www.themoviedb.org/t/p/w1280/9d1sCoMSGJZtghS2X9us1h9u8lW.jpg",
            Rating = 8.7,
            Year = 2014,
        ...
        new VideoCardViewModel
        {
            ...
            BackdropUrl = "https://media.themoviedb.org/t/p/w1000_and_h563_face/63FA8vwSZnXkGxedrDQwni4JuZN.jpg",
        ...
        new VideoCardViewModel
        {
            ...
            BackdropUrl = "https://media.themoviedb.org/t/p/w1000_and_h563_face/fm6KqXpk3M2HVveHwCrBSSBaO0V.jpg",
        ...
        new VideoCardViewModel
        {
            ...
            Duration = "5 сезонів",
            AgeRating = "18+",
            TrailerUrl = "https://www.youtube.com/watch?v=suobrMPBLlo",
        ...
        new VideoCardViewModel
        {
            ...
            BackdropUrl = "https://media.themoviedb.org/t/p/w1000_and_h563_face/o5fynWCt3sd7EnL1kq0hpGJKYEw.jpg",
            ...
            Duration = "2 сезони",
            AgeRating = "18+",
            TrailerUrl = "https://www.youtube.com/watch?v=aaQSScwZPbA",
        ...
        new VideoCardViewModel
        {
            ...
            BackdropUrl = "https://media.themoviedb.org/t/p/w1000_and_h563_face/apMfdiVZkAgE5w9jywQnbMi8KEo.jpg",
    ...
    public List<VideoCardViewModel> GetAllVideos()
    {
        return _videos;
    }

    public bool ToggleSavedStatus(int videoId)
    {
    ...
    {
        var genres = Enum.GetValues(typeof(VideoGenre))
        .Cast<VideoGenre>()
        .Select(genre => new GenreViewModel
        {
    ...
    {
        var genreVideos = _videos
            .Where(v => v.Genres != null && v.Genres.Any(
                g => string.Equals(g, genre, StringComparison.OrdinalIgnoreCase)))
            .ToList();
        return Task.FromResult(genreVideos);
    }

    public Task<List<HeroItemViewModel>> GetHeroSlidersAsync(string locale, int? userId = null)
    {
        var heroItems = new List<HeroItemViewModel>
        {
        ...
        return Task.FromResult(heroItems);
    }

    public Task<List<VideoCardViewModel>> GetNewReleasesAsync(string locale, int? userId = null)
    {
        var newReleases = _videos
        .Where(v => v.ReleaseDate == null || v.ReleaseDate <= DateTime.Now) 
    ...
    {
        var popularVideos = _videos
        .OrderByDescending(v => v.Rating)
        .Take(15)
        .ToList();

        return Task.FromResult(popularVideos);
    }

    public Task<List<string>> GetSearchSuggestionsAsync(string query, string locale, int limit)
    {
        if (string.IsNullOrWhiteSpace(query)) return Task.FromResult(new List<string>());

        var suggestions = _videos
    ...
    {
        var sliderVideos = _videos.Take(8).ToList();
        return Task.FromResult(sliderVideos);
    }

    public Task<List<VideoCardViewModel>> GetTrendingAsync(string locale, int? userId = null)
    {
        var trendingVideos = _videos
        .Where(v => v.Year >= 2020)
        .OrderByDescending(v => v.Rating)
        .Take(15)
        .ToList();

        return Task.FromResult(trendingVideos);
    }
    ...
    public Task<Dictionary<string, List<VideoCardViewModel>>> GetUpcomingReleasesAsync(string locale)
    {
        ...
        var upcomingDictionary = _videos
            .Where(v => v.ReleaseDate != null && v.ReleaseDate > DateTime.Now)
            .OrderBy(v => v.ReleaseDate)
            .GroupBy(v => v.ReleaseDate.Value.Date)
            .ToDictionary(
                g => g.Key.ToString("dd MMM, yyyy", culture).Replace(".", ""),
                g => g.ToList()
            );

        return Task.FromResult(upcomingDictionary);
    }
    ...
    public Task<List<VideoCardViewModel>> GetVideosByGenresAsync(List<string> genreCodes, string locale, int? userId = null)
    {
        if (genreCodes == null || !genreCodes.Any())
            return Task.FromResult(new List<VideoCardViewModel>());

        var result = _videos
            .Where(v => v.Genres != null &&
                        v.Genres.Any(g => genreCodes.Contains(g, StringComparer.OrdinalIgnoreCase)))
            .ToList();
    ...
    {
        var weeklyHits = _videos
            .OrderByDescending(v => v.Rating)
            .Take(7)
            .ToList();

        return Task.FromResult(weeklyHits);
    }

    public Task<SearchResultsViewModel> SearchVideosAsync(string? query, int? genreId, string? sortBy, int page, int pageSize, string locale, int? userId)
    {
    ...
    public Task<List<VideoCardViewModel>> GetPostponedVideosAsync(int userId, string locale)
    {
        var postponed = _videos
            .Where(v => v.IsSavedForLater)
            .ToList();

        return Task.FromResult(postponed);
    }
}